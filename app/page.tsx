...
export default function Home(){
 ...
 const game=games[gameId],variationCurrentFen=variation?.currentFen??null,fen=variationCurrentFen??game.fens[ply],board=new Chess(fen),mainlineEvaluation=results[ply],current=variation?variationEvaluation:mainlineEvaluation,move=game.moves[ply-1];
 ...
 const panelBestMove=current?.best?san(fen,current.best):'—';
 const engineBusy=variation?variationBusy:busy,engineStatus=variation?variationStatus:status,engineProgress=variation?(variationEvaluation?100:variationBusy?45:0):Object.keys(results).length/game.fens.length*100;
 const variationArrow=variationCurrentFen?variationBestMoveArrow(variationCurrentFen,variationEvaluation,variationBusy):null;
 ...
 function runVisibleAnalysis(){if(variationRef.current){if(variationBusy)stopVariationAnalysis();else void analyzeVariationPosition(fen);}else if(busy)stop();else void analyze();}
 ...
 <div className="graph-card"><div className="card-heading"><h3>Evaluation timeline</h3><span>White’s perspective</span></div><svg viewBox="0 0 600 90" role="img" aria-label="Game evaluation graph"><line x1="0" x2="600" y1="45" y2="45" stroke="#39433d" strokeDasharray="4 5"/><polyline fill="none" stroke="#b9eb83" strokeWidth="2" points={Object.entries(results).map(([i,e])=>`${Number(i)/Math.max(1,game.moves.length)*600},${45-Math.max(-8,Math.min(8,e.cp/100))*5}`).join(' ')}/><line x1={ply/Math.max(1,game.moves.length)*600} x2={ply/Math.max(1,game.moves.length)*600} y1="0" y2="90" stroke="#8c958e"/>{game.fens.map((_,i)=><rect key={i} x={i/game.fens.length*600} width={600/game.fens.length} height="90" fill="transparent" onClick={()=>selectMainline(i)}><title>Position {i}</title></rect>)}</svg><div className="graph-foot"><span>Opening</span><span>{Object.keys(results).length?'Click the chart to explore':'Run analysis to reveal the evaluation'}</span><span>End</span></div></div>
  ...
  {variation?<section className="variation-engine"><div className="variation-engine-context"><strong>Analysis Variation</strong><span>{board.isCheckmate()?'Checkmate':board.isStalemate()?'Stalemate':board.isDraw()?'Draw':`${board.turn()==='w'?'White':'Black'} to move`}</span></div><VariationEngineFeedback evaluation={variationEvaluation} busy={variationBusy} bestMove={panelBestMove} positionState={board.isCheckmate()?'Checkmate':board.isStalemate()?'Stalemate':board.isDraw()?'Draw':'No continuation available'}/></section>:<EngineReviewFeedback review={selectedReview}/>} 
  <div className="engine-options"><label>Search depth <select value={depth} disabled={engineBusy} onChange={e=>changeDepth(Number(e.target.value))}><option value="10">10 · Quick</option><option value="14">14 · Balanced</option><option value="18">18 · Deep</option></select></label><button className="analyze" onClick={runVisibleAnalysis}>{engineBusy?<Stop size={15}/>:<Play size={15} fill="currentColor"/>}{engineBusy?'Stop analysis':variation?variationEvaluation?'Analyze position again':'Analyze position':engineProgress===100?'Analyze again':'Analyze game'}</button></div>
